...
//            log.info("Automatic upgrade of Embedded Database is currently not supported. To update to a new schema, you will need to delete the database at {}, and restart ValueReporter. ", jdbcUrl);
            if (setupNewDatabase) {
                log.info("You have decided to setup new database. Removing the old database.");
                String filepath = findHSQLDbPath(jdbcUrl);

                Path rootPath = Paths.get(filepath);


                if (Files.exists(rootPath)) {
                    log.info("Path does exist: {}", filepath);
                    try {
                        ...
                        Files.walk(rootPath, FileVisitOption.FOLLOW_LINKS)
                                ...
                                .forEach(File::delete);
                        int i = 0;
                        while (Files.exists(rootPath) && i < 10) {
                            log.info("Waiting for files to be deleted.");
                            i++;
                            try {
                                Thread.sleep(100);
                    ...
                    log.info("Path is not found. {}", filepath);
                }
            }
//            return;
        }
    ...
    String findHSQLDbPath(String jdbcUrl) {
        String filepath = null;
        int fileStr = jdbcUrl.indexOf("file:");
        int lastSlash = jdbcUrl.lastIndexOf("/");
        if (fileStr > 0 && fileStr < jdbcUrl.length()) {
            if (lastSlash > 0 && lastSlash < jdbcUrl.length()) {
                filepath = jdbcUrl.substring(fileStr + 5, lastSlash);
            }
        }
        return filepath;
    }
...
//            Class.forName(jdbcDriverClassName);
    ...
    private void createObservedKeys(Connection connection) throws SQLException {
        String tableSql = "CREATE TABLE ObservedKeys(\n" +
                ...
                "      CONSTRAINT AK_Prefix_MethodName UNIQUE (serviceName, methodName)\n" +
                "    );";
    ...
    private void createImplementedMethod(Connection connection) throws SQLException {
        String tableSql = ("CREATE TABLE ImplementedMethod (" +
                ...
                "  serviceName varchar(255) NOT NULL," +
                "  methodName varchar(255) NOT NULL," +
                "  CONSTRAINT IMK_PrefixMethodName UNIQUE(serviceName, methodName)"+
                "); ");
    ...
    private void insertValues(Connection connection)  {
        ...
        try {
            queryRunner.update(connection, "insert into ObservedMethod (serviceName,methodName, startTime, endTime, duration) values ('inital', 'com.valuereporter.test', '2014-05-13 12:02:43.296','2014-05-13 12:02:43.596',300);");
            queryRunner.update(connection, "insert into ImplementedMethod (serviceName,methodName) values ('inital', 'com.valuereporter.test');");
            queryRunner.update(connection, "insert into ObservedKeys(serviceName, methodName) values ('initial', 'com.valuereporter.test');");
    ...
    private void createWhydahTables(Connection connection) throws SQLException {
        createWhydahUserSession(connection);
        createWhydahUserLogon(connection);

    }

    private void createWhydahUserSession(Connection connection) throws SQLException {
        String tableSql = ("CREATE TABLE userSession (" +
                "  id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY," +
                "  startTime TIMESTAMP ," +
    ...
    private void createWhydahUserLogon(Connection connection) {
